...
describe('ApiPathHelper', () => {
  it('should create valid paths', async () => {
    // TODO: make executable for windows
    const basePath = ApiPathHelper.basePath('foo');
    expect(basePath).toContain('apps/foo');

    const srcPath = ApiPathHelper.srcPath('foo');
    expect(srcPath).toContain('apps/foo/src');

    const appPath = ApiPathHelper.appPath('foo');
    expect(appPath).toContain('apps/foo/src/app');

    const modulesPath = ApiPathHelper.modulesPath('foo');
    expect(modulesPath).toContain('apps/foo/src/app/modules');

    const moduleAPath = ApiPathHelper.modulePath('foo', 'a');
    expect(moduleAPath).toContain('apps/foo/src/app/modules/a');

    const envPath = ApiPathHelper.envPath('foo');
    expect(envPath).toContain('apps/foo/src/environments');

    const assetsPath = ApiPathHelper.assetsPath('foo');
    expect(assetsPath).toContain('apps/foo/src/assets');

    const dbPath = ApiPathHelper.dbPath('foo');
    expect(dbPath).toContain('apps/foo/src/database');
